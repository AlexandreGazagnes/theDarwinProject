...
class EvolutionAlgo1D() :
    ...
    def __init__(self, 
        ...
        parents = [np.random.rand() + np.random.randint(*interval) for _ in range(seed_parents)]
        self.current_population = [ (
                                    round(x, self._round), 
                                    round(self.funct(x), self._round) , "first", 
                                    self.year) for x in parents]
        self.original_population = [(i, j, k, l) for i, j, k, l in self.current_population]
    ...
    def _sort_current_population(self) : 
        ...
        logger.debug("called")
        # first # sort regarding the fuct
        reverse = False if self.objective == "min" else True
        self.current_population = \
                sorted( self.current_population, key=lambda i : i[1], reverse=reverse)
    ...
    def _procreate(self) : 
        ...
        # rand childs
        random_childs_x = [np.random.rand() + np.random.randint(*self.interval) for _ in range(N_random_childs)]
        random_childs = [(
                            round(x, self._round), 
                            round(self.funct(x), self._round), 
                            "random", 
                            self.year) for x in random_childs_x]
        ...
        for _ in range(N_normal_child-1) : 
            ...
            random_parents = np.random.choice(self.current_population_x, size=numb_parents )
        ...
        normal_childs = [
                            ...
                            round(self.funct(x), self._round), 
                            "normal",
                            self.year) for x in normal_childs_x]
